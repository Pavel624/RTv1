...
void init_shapes(t_rtv *rtv)
{
	...
	t_vector3 center = new_vector3(0, 0, 30);
	t_color	color = set_color(255, 255, 255);
	rtv->sphere = new_sphere(center, 10., color);

	rtv->cam->pos = new_vector3(0, 0, 0);
	rtv->cam->dir = new_vector3(0, 0, 1);

	rtv->light.pos = new_vector3(30, 30, 5);
	rtv->light.intensity = set_color(255, 255, 255);
}
...
//			camera->dir = sub_vector3(screen, camera->pos);
...
//	}
	...
	mlx_loop_hook(rtv->window, rtv_pthread, rtv);
	mlx_loop(rtv->mlx);
	return (0);
}
